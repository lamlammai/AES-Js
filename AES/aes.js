class Aes {
    // input là mảng 1 chiều
    
    static cipher(inp, w) {
        const Nb = 4;
        const Nr = w.length/Nb-1;
        let state = [[], [], [], []];
        // dựng bảng trạng thái, từ input ra bằng trạng thái
        for(let i = 0; i < 4*Nb; i++) {
            state[i%4][Math.floor(i/4)] = inp[i] || '';
        }
        state = Aes.addRoundKey(state, w, 0, Nb);
        for(let round = 1; round < Nr; round++) {
            state = Aes.subBytes(state, Nb);
            state = Aes.shiftRows(state, Nb);
            state = Aes.mixColumns(state, Nb);
            state = Aes.addRoundKey(state, w, round, Nb);
        }
        state = Aes.subBytes(state, Nb);
        state = Aes.shiftRows(state, Nb);
        state = Aes.addRoundKey(state, w, Nr, Nb);
        const output = new Array(4*Nb);  // convert state to 1-d array before returning [§3.4]
        for (let i=0; i<4*Nb; i++)
        {
            output[i] = state[i%4][Math.floor(i/4)];
        } 

        return output;
    }
    static addRoundKey(state, w, rnd, Nb) {
        for(let row = 0; row < 4; row++) {
            for(let col = 0; col < Nb; col++) {
                state[row][col] ^= w[rnd*4 + col][row];
            }
        }
        return state;
    }
    static subBytes(s, Nb) {
        for(let row = 0; row < 4; row++) {
            for(let col = 0; col < Nb; col++) {
                s[row][col] = Aes.sBox[s[row][col]];
            }
        }
        return s;
    }
    // 1- ghi hàng sau  vào mảng temp 
    static shiftRows(s, Nb) {
        const temp = new Array(4);
        for(let row = 1; row < 4; row++) {
            // bien temp ghi hang sau khi shift;
            for(let col = 0; col < 4; col++) {
                temp[col] = s[row][(col + row) % Nb];
            }
            for(let col = 0; col < 4; col++) {
                s[row][col] = temp[col];
            }
        }
        return s;
    }
    static mixColumns(s, Nb) {
        
        // 2   3   1   1
        // 1   2   3   1
        // 1   1   2   3
        // 3   1   1   2
        for (let c=0; c<Nb; c++) {
            // a là 1 cột
            const a = new Array(Nb);  // 'a' is a copy of the current column from 's'
            const b = new Array(Nb);  // 'b' is a•{02} in GF(2^8)
            for (let r = 0; r < 4; r++) {
                a[r] = s[r][c];
                b[r] = s[r][c]&0x80 ? s[r][c]<<1 ^ 0x011b : s[r][c]<<1;
            }
            // a[n] ^ b[n] is a•{03} in GF(2^8)
            s[0][c] = b[0] ^ a[1] ^ b[1] ^ a[2] ^ a[3]; // {02}•a0 + {03}•a1 + a2 + a3
            s[1][c] = a[0] ^ b[1] ^ a[2] ^ b[2] ^ a[3]; // a0 • {02}•a1 + {03}•a2 + a3
            s[2][c] = a[0] ^ a[1] ^ b[2] ^ a[3] ^ b[3]; // a0 + a1 + {02}•a2 + {03}•a3
            s[3][c] = a[0] ^ b[0] ^ a[1] ^ a[2] ^ b[3]; // {03}•a0 + a1 + a2 + {02}•a3
        }
        return s;
    }
    /**
     * Perform key expansion to generate a key schedule from a cipher key [§5.2].
     *
     * @param   {number[]}   key - Cipher key as 16/24/32-byte array.
     * @returns {number[][]} Expanded key schedule as 2D byte-array (Nr+1 × Nb bytes).
     */
    // w là mảng chứa những roundkey
    static keyExpansion(key) {
        const Nb = 4;            // block size (in words): no of columns in state (fixed at 4 for AES)
        const Nk = key.length/4; // key length (in words): 4/6/8 for 128/192/256-bit keys
        const Nr = Nk + 6;       // no of rounds: 10/12/14 for 128/192/256-bit keys
        // 44
        const w = new Array(Nb*(Nr+1)); // mảng để chứa roundkey
        let temp = new Array(4);
        // initialise first Nk words of expanded key with cipher key// lấy dãy key cũ
        for (let i=0; i<Nk; i++) {
            const r = [ key[4*i], key[4*i+1], key[4*i+2], key[4*i+3] ];
            w[i] = r;
        }

        // expand the key into the remainder of the schedule
        for (let i = Nk; i < (Nb * ( Nr + 1 )); i++) {
            w[i] = new Array(4);
            for (let t = 0; t < 4; t++) {
                temp[t] = w[i-1][t];
            }
            // each Nk'th word has extra transformation
            if (i % Nk == 0) {
                temp = Aes.subWord(Aes.rotWord(temp));
                for (let t=0; t<4; t++) temp[t] ^= Aes.rCon[i/Nk][t];
            }
            // 256-bit key has subWord applied every 4th word
            else if (Nk > 6 && i%Nk == 4) {
                temp = Aes.subWord(temp);
            }
            // xor w[i] with w[i-1] and w[i-Nk]
            for (let t=0; t<4; t++) {
                w[i][t] = w[i-Nk][t] ^ temp[t];
            }
        }
        return w;
    }
    static rotWord(w) {
        const tmp = w[0];
        for(let i = 0; i < 3; i++) {
            w[i] = w[i+1];
        }
        w[3] = tmp;
        return w;
    }
    static subWord(w) {
        // note: w[i]: cung la so thu tu trong bang sbox
        for(let i = 0; i < 4; i++) {
            w[i] = Aes.sBox[w[i]];
        }
        return w;
    }
}
Aes.sBox = [
    ,
];// rCon is Round Constant used for the Key Expansion [1st col is 2^(r-1) in GF(2^8)] [§5.2]
Aes.rCon = [
    [ 0x00, 0x00, 0x00, 0x00 ],
    [ 0x01, 0x00, 0x00, 0x00 ],
    [ 0x02, 0x00, 0x00, 0x00 ],
    [ 0x04, 0x00, 0x00, 0x00 ],
    [ 0x08, 0x00, 0x00, 0x00 ],
    [ 0x10, 0x00, 0x00, 0x00 ],
    [ 0x20, 0x00, 0x00, 0x00 ],
    [ 0x40, 0x00, 0x00, 0x00 ],
    [ 0x80, 0x00, 0x00, 0x00 ],
    [ 0x1b, 0x00, 0x00, 0x00 ],
    [ 0x36, 0x00, 0x00, 0x00 ],
];
module.exports = Aes;