...
class AesECB extends Aes {
    static encrypt(plaintext, password, nBits) {
        ...
        if(nBits != 128 && nBits != 192 && nBits != 256) {
            throw new Error('key size not corrrect');
        }
        
        for(let i = 0, j = 0; i < plaintext.length; i++) {
            plainTextSegments[j] += plaintext[i]; // Cái này là chia thành từng đoạn 16 ký tự này
            if(i % 16 == 0) {
                j++;
            }
        }   

        const nBytes = nBits/8; // no bytes in key (16/24/32) cái này là độ dài khóa
        const pwBytes = new Array(nBytes);
        for (let i=0; i < nBytes; i++) { // cái này là lấy ra đủ độ dài khóa, nếu thiếu thì chèn 0
        ...
        // const [ nonceMs, nonceSec, nonceRnd ] = [ 0, 0, 0 ];
    ...
    */
    static base64Encode(str) {
        ...
        if (typeof Buffer != 'undefined') return new Buffer.from(str, 'binary').toString('base64'); // Node.js
    ...
    */
    static base64Decode(str) {
        ...
        if (typeof Buffer != 'undefined') return new Buffer.from(str, 'base64').toString('binary'); // Node.js
...
const arr = AesECB.encrypt("nguyen duy", "duy", 128);
console.log(arr);
const plant = AesECB.decrypt(arr, "duy", 128)
console.log(plant);
